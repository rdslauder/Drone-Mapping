...
def timedRename(): 
    ...
    # Create an empty list to store tuples of (filename, time)
    files = []
...
def accuracyAssessment():
    ...
    while True:
        ...
        # ValueError occurs if the numbers of drone images do not match the expected number of images from the flight plan.
        # Duplicates can happen during battery changes, or when wind blows the drone well off course and it must restart the flight from where it was last on course.
        except ValueError:
            #pass # do not print the ValueError message
            print("\n\n\n\n\n" + 
                "Accuracy assessment failed." + "\n" + 
                "The number of captured images do not match the flight plan, likely due to duplicate images." + "\n" + 
...
# labelled plots saved so that if errors occur in the accuracy assessment, they can be referred to
prePlotCoordinatesLabels()
postPlotCoordinatesLabels()

accuracyAssessment() 
# if indices do not match (number of photos doesn't match the flight plan), compare the plots to find the duplicate photo.
